...
export function setPrompt(promptTextStr, targetDisplayStr) {
    ...
    if (promptEl)
        promptEl.textContent = promptTextStr;
    if (targetEl)
        targetEl.textContent = targetDisplayStr;
}
...
export function showGameOver(finalScoreMsg) {
    ...
    if (msgEl)
        msgEl.textContent = finalScoreMsg;
    if (overlay)
...
export function playShootCursorAnimation() {
    ...
    const cross = el(IDS.crosshair);
    if (dot)
        dot.classList.add(SHOOT_CURSOR_CLASS);
    if (cross)
        cross.classList.add(SHOOT_CURSOR_CLASS);
    setTimeout(() => {
